...
import os
import numpy as np
from embed_process_data import convert_token_to_matrix, split_train_and_test_data, extract_content_map
from embed_gru import GRU_MODEL as gru_model
# from embed_evaluate import  find_correct_predictions
import pdb
...
def write_student_sample(sample_writer, student, sessions, stud_input,
    ...
    for i, label in enumerate(actual):
        ...
        # else:
        readable_input = create_readable_list_from_token(
...
def create_readable_list_from_token(vect, index_to_content_map):
    '''
    ...
    for i in vect:
        if i!=0:
            content = index_to_content_map[ int(vect[0].detach())]
...
def run_inference():
    ...
    model = gru_model(input_dim=input_dim,
                      ...
                      # include correct always false
                      # until solution figured out for embedding
